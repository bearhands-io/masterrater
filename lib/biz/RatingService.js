...
function RatingService(app) {
	...
	this.getRatesMulti = function(company, fromZips, toZip, weight, renderCallback) {
		var rates = [];
		var callbackCount = 0;
		...
		for(var i = 0, len = fromZips.length; i < len; i++) {
			getRates(company, fromZips[i], toZip, weight, callback);
		}
	}
	...
	*/
	function getRates(company, fromZip, toZip, weight, callback) {
		...
		for(var i = 0, len = RATING_MEHODS.length; i< len; i++) {
			...
			ratingMethod(company, fromZip, toZip, weight, ratingMethodCallback);
		}

		return rates;
	}

	function cts(company, fromZip, toZip, weight, callback) {
		if(!company || !company.ctsUID) {
		...
		httpsRequest(http, query, parseResponseCallback);
	}

	function customCarrierRates(company, fromZip, toZip, weight, callback) {
		...
		determineDistanceGoogle(company, fromZip, toZip, successCallback, errorCallback);
	}

	// this function should contain all google's api references
	function determineDistanceGoogle(company, fromZip, toZip, successCallback, errorCallback) {
		if(!company || !company.googleApiKey) {
		...
		httpsRequest(https, query, parseResponseCallback);
	}

	function httpsRequest(protocol, query, responseCallback) {
		...
		protocol.request(query, requestCallback).end();
	}
}
